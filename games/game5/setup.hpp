...
public:
    ...
    void casino_move(int choice, bool debug = false){
        if (debug) print_var(choice);
        if (this->winning_slot == NOT_DEFINED) this->winning_slot = choice; // game start
        else if (this->winning_slot != choice && k-- > 0) this->winning_slot = choice;
    ...
    void gambler_move(int bet, int slot, bool debug = false){
        if (bet == 0) game_over();
        assert(bet > 0 && bet <= 3);
